...
class PrtoblematicPairsDataLoader(object):
    ...
    def markPairs(self, idx):
        ...
        grouped = tempdf.groupby(['src', 'dest', 'hash', 'src_host', 'dest_host', 'src_site', 'dest_site']).agg(
    ...
    def fixMissingMetadata(self, probdf, idx):
        ...
        df1 = pd.merge(metadf[['host', 'ip', 'site']], probdf[[
                       'src', 'hash']], left_on='ip', right_on='src', how='right')
        df2 = pd.merge(metadf[['host', 'ip', 'site']], probdf[[
